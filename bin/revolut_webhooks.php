 ...
 *
 * No database gateway IDs are used. Foundry identifies the integrations by
...
if ($projectRoot === false || !is_file($projectRoot . '/vendor/autoload.php')) {
    fwrite(STDERR, "Foundry vendor/autoload.php was not found. Run composer install first.\n");
    exit(1);
}
...
try {
    ...
    $client = new Client([
        ...
        'headers' => [
            ...
            'User-Agent' => 'Foundry-Revolut-Webhooks/1.0',
        ],
    ]);
...
function updateEnvValue(string $envFile, string $name, string $value): void
{
    ...
    if ($handle === false) {
        throw new RuntimeException('Unable to open Foundry .env for writing.');
    }

    try {
        if (!flock($handle, LOCK_EX)) {
            throw new RuntimeException('Unable to lock Foundry .env.');
        }

        rewind($handle);
        $contents = stream_get_contents($handle);
        if ($contents === false) {
            throw new RuntimeException('Unable to read Foundry .env.');
        }
        ...
        if (preg_match($pattern, $contents) === 1) {
            ...
            if ($updated === null) {
                throw new RuntimeException('Unable to update Foundry .env.');
            }
        } else {
        ...
        if (!ftruncate($handle, 0) || fwrite($handle, $updated) === false || !fflush($handle)) {
            throw new RuntimeException('Unable to save Foundry .env.');
        }
...
function usage(int $status = 0): never
{
    ...
    fwrite($stream, <<<TEXT
Foundry Revolut webhook maintenance
